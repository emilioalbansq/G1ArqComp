...
1. ¿Qué resultado se tiene al final en el registro del acumulador?
   ...
   Respuesta:
   El valor final en el acumulador es -3
   $[AC] \leftarrow -3$
...
2. ¿Se sobrescribe algún registro como resultado de la ejecución? Si
   ...
   Respuesta:
   - $[940] \leftarrow -3$ (Primero se sobrescribió con 3, y al final con -3)
   - $[941] \leftarrow 6$
   - $[AC] \leftarrow -3$
...
3. ¿Cuál es el valor del bit de Carry? ¿Qué operación modifica el bit?
   ...
   - No se produce desbordamiento (overflow) en ninguna de las
     operaciones aritméticas. La resta ($5 - 2$) y el desplazamiento a
     la izquierda ($3 \times 2$) generan resultados que caben
     perfectamente dentro de la capacidad de 40 bits del acumulador de
     la máquina IAS, por lo que el bit de Carry para indicar un error
     de magnitud se mantiene en 0.
...
| Nombre         | Email                     | Rol        |
|----------------+---------------------------+------------|
| Emilio Alban   | emilio.alban@epn.edu.ec   | Líder      |
| Ariel Montufar | ariel.montufar@epn.edu.ec | Integrante |
| Karen Suarez   | karen.suarez01@epn.edu.ec | Integrante |
|                |                           |            |
|----------------+---------------------------+------------|
